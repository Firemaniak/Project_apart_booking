...
# Раздача media-файлов вручную (static() из django.conf.urls.static не работает при DEBUG=False)
urlpatterns += [
    path(f'{settings.MEDIA_URL.lstrip("/")}<path:path>', serve, {'document_root': settings.MEDIA_ROOT}),
]

# Раздача фронтенда — должно быть в самом конце, это "поймай всё остальное"
FRONTEND_DIR = BASE_DIR / 'frontend'
